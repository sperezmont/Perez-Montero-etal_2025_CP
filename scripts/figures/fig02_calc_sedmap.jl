...
# Plot
fig = Figure(resolution=(600, 900), fonts=(; regular="Makie"), fontsize=fontsize)
...
contour!(ax1, lon_pd, lat_pd, mask_pd, levels=[0.5], color=:darkorange, linewidth=2)
contour!(ax1, lon_ice, lat_ice, mask_lgm, levels=[0.5], color=:cornflowerblue, linewidth=2)
xlims!(ax1, (0, 360))
...
Colorbar(fig[1, 2], hm, height=Relative(1/3), label=L"Sediment thickness$\,$", 
       ticks=([0, 1, 2, 3, 4], convert_strings_to_latex([L"0$\,$", L"10\,m$\,$", L"100\,m$\,$", L"1\,km$\,$", L"10\,km$\,$"])),
        vertical=true, flipaxis = true)

ax = Axis(fig[2, 1], xlabel=L"Sediment thickness$\,$", ylabel=L"Frequency (points)$\,$", xgridvisible=false, ygridvisible=false, yaxisposition=:right)
hidespines!(ax, :t, :l)
...
xlims!(ax, (0, 4))
hist!(ax, z_pd_hist, color=:darkorange, bins=calc_bins(length(z_pd_hist)), strokewidth=0.5, strokecolor=:black, direction=:y, label=L"\mathrm{PD~continents}")
hist!(ax, z_lgm_hist, color=(:cornflowerblue, 0.0), bins=calc_bins(length(z_lgm_hist)), strokewidth=2, strokecolor=:cornflowerblue, direction=:y, label=L"\mathrm{LGM~ice~sheets}")

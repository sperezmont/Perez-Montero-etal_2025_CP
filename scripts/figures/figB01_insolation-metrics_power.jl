...
# ax = Axis(fig[5, 1], xlabel=time_label, ylabel=L"$P$ (kyr)", xgridvisible=false, ygridvisible=false, yaxisposition=:right, yscale=Makie.pseudolog10)
# ax.xticks = xticks_time
# ax.yticks = (ticks_periods, convert_strings_to_latex(ticks_periods))
# ylims!(ax, (10, 70))
# xlims!(ax, xlims_time_long)
...
hidexdecorations!(ax)
hidespines!(ax, :b, :t)
text!(ax, -2980, 5.3, text=L"(c) ISI275$\,$", align=(:left, :center))
...
# Panel e. ISI400 
ax = Axis(fig[9, 1], ylabel=L"E (GJ/m²)$\,$", xlabel=time_label, xgridvisible=false, ygridvisible=false)
ax.xticks = xticks_time

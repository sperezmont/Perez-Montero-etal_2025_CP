...
function create_coi(t, freqs; sigma=pi)
    t_coi, p_coi = calc_coi(t, freqs, 2 * pi / sigma)  # convert rad/s to Hz (for cf)

    t1, p1 = t_coi[1:Int(length(t_coi) / 2)], p_coi[1:Int(length(p_coi) / 2)]
    t2, p2 = t_coi[Int(length(t_coi) / 2)+1:end], p_coi[Int(length(p_coi) / 2)+1:end]

    return t_coi, p_coi, t1, p1, t2, p2
end

function plot_coi!(axis, color, t, freqs; sigma=pi, transparency=0.3, linestyle=:dash, factor=1/1000)
    t_coi, p_coi, t1, p1, t2, p2 = create_coi(t, freqs, sigma=sigma)

    t1, p1 = t1 .* factor, p1 .* factor
    t2, p2 = t2 .* factor, p2 .* factor
    t = t .* factor

    lines!(axis, t1, p1, color=color, linestyle=linestyle)
    lines!(axis, t2, p2, color=color, linestyle=linestyle)
    band!(axis, t1, p1, 10000, color=(color, transparency))
    band!(axis, -2e6:t1[end], 0, 10000, color=(color, transparency))
    band!(axis, t2, p2, 10000, color=(color, transparency))
    band!(axis, t2[end]:1e6, 0, 10000, color=(color, transparency))
    return nothing
